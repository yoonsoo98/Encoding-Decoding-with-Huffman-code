...
element huffman_tree(input_huff *huff)
{
	...
	for (i = 0; i<n; i++) {
		...
		memset(node->bits, 0, sizeof(int)*MAX_BIT); //비트를 0000000000으로 초기화
		node->bit_size = 0;
	...
	for (i = 1; i<n; i++) {
		...
		e.ptree = x;
		x->data = NULL; // 새로 생성한 것이므로 data값 없음
		e.key = x->key = e1.key + e2.key;
		memset(x->bits, 0, sizeof(int)*MAX_BIT);//비트를 0000000000으로 초기화
		x->bit_size = 0;
	...
	return e;
	//	destroy_tree(e.ptree); -> 허프만 코드로 encoding해야 하므로 destroy하지 않음
}
...
{
	if (node->l==NULL&&node->r==NULL) {//노드가 단말노드일 때
		char alpha = node->data;
		int index;
		switch (alpha) {
		case 'a':
			index = 0;
...
void huffman_encoding(char *str, bits_stream *bits_str)
{
	...
	for (int i = 0; i < length; i++) {
		...
		for (int i = interval,k=0; k < m_bit_size[index]; k++,i++,interval++) {
			bits_str->stream[i] = m_LUT[index][k];
		}
	}
...
void main()
{
	...
	memset(bits_str1->stream, -1, sizeof(int)*MAX_BIT * MAX_CHAR); // 초기값을 -1로 설정
	bits_str1->length = 0;
